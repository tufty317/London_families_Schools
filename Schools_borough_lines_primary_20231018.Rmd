...
```{r fig_London_SF_Prim_schools_line_5, fig.height = 5.56, fig.width = 9,  echo=FALSE, warning=FALSE, message=FALSE}
...
London_SF_prim_schools_agg_lines <- agg_input_london_SF_primary_schools %>%
  ...
  labs(title= "State-funded full-time primary school pupils by Zone, London, 2009 - 2022", 
       caption = paste0("Source: ONS, Chart: GLA demography"))
London_SF_prim_schools_agg_lines
...
```{r fig_London_Prim_indep_schools_line2, fig.height = 5.56, fig.width = 9,  echo=FALSE, warning=FALSE, message=FALSE}

catcolour2 = rev(c('#d82222', '#6da7de'))

# Aggregate by Zone of London

agg_input_london_indep_primary_schools <- input_all_schools_data %>%
  filter(grepl('E09', new_la_code)) %>%
  filter(la_name.x != "City of London") %>%
  filter(age < 11) %>%
...
London_indep_schools_agg_lines <- agg_input_london_indep_primary_schools %>%
   ggplot(aes(x = period_start, y = agg_full_time/1000, group = cityORcounty_name, color = cityORcounty_name, 
             text = paste("Year  :", period_start,
                            "<br>Zone : ", cityORcounty_name,
                            "<br>Headcount : ", round(agg_full_time, digits = 0))
  ...
  geom_line(size = 1) + 
  scale_color_manual(name='Zone:', values = rev(catcolour2)) +
  labs(title= "State-funded full-time independent primary school pupils by Zone, London, 2009 - 2022", 
       caption = paste0("Source: ONS, Chart: GLA demography"))
London_indep_schools_agg_lines
...
```
```{r fig_London_Prim_indep_schools_line3, fig.height = 5.56, fig.width = 9,  echo=FALSE, warning=FALSE, message=FALSE}

catcolour2 = rev(c('#d82222', '#6da7de'))

all_London_schools_wide <- input_all_schools_data %>%
  filter(grepl('E09', new_la_code)) %>%
  filter(la_name.x != "City of London") %>%
  filter(age < 11) %>%
  pivot_wider(
    names_from = type,
    values_from = c(full_time)) %>%
  group_by(period_start, cityORcounty_name) %>% 
  summarise(agg_independent = sum(Independent), agg_total = sum(Total)) %>%
  mutate(indepProp = agg_independent/agg_total) %>%
      data.frame()


# Aggregate by Zone of London

London_indep_schools_prop_lines <- all_London_schools_wide %>%
   ggplot(aes(x = period_start, y = indepProp, group = cityORcounty_name, color = cityORcounty_name, 
             text = paste("Year  :", period_start,
                            "<br>Zone : ", cityORcounty_name,
                            "<br>Headcount : ", round(indepProp, digits = 0))
  )) +
  theme_gla() +
  theme(plot.title = element_text(size = 16, hjust = 0.5)) +
  theme(legend.position = "right") +
  geom_line(size = 1) + 
  scale_color_manual(name='Zone:', values = rev(catcolour2)) +
  labs(title= "Proportion of independent primary school pupils by Zone, London, 2009 - 2022", 
       caption = paste0("Source: ONS, Chart: GLA demography"))
London_indep_schools_prop_lines
...
London_SF_prim_schools_agg_ind2009_lines <- agg_input_london_SF_primary_schools %>%
  ...
  mutate(period_start_num = as.numeric(period_start)) %>%
   ggplot(aes(x = period_start_num, y = indexed_headcount, group = cityORcounty_name, color = cityORcounty_name
             , text = paste("Year  :", period_start,
                            "<br>Zone : ", cityORcounty_name,
  ...
  theme(legend.position = "right") +
  # scale_x_continuous(breaks = seq(2009, 2022, 3), labels = c("2009-10", "2012-13", "2015-16", "2018-19", "2021-22"))  +
 ...
 scale_color_manual(name='Zone:', values = rev(catcolour2)) +
  labs(title= "State-funded full-time primary school pupils by Zone, London, 2009 - 2022", 
       ...
       caption = paste0("Source: ONS, Chart: GLA demography"))
London_SF_prim_schools_agg_ind2009_lines
...
London_SF_prim_schools_agg_borough_lines <- agg_input_london_SF_primary_schools_borough %>%
  ...
  labs(title= "State-funded full-time primary school pupils, London, 2009 - 2022", 
       caption = paste0("Source: ONS, Chart: GLA demography"))
#London_SF_prim_schools_agg_borough_lines

ggplotly(London_SF_prim_schools_agg_borough_lines, tooltip = "text") %>%
  style(hoverlabel = list(bgcolor = "white")) %>%
  layout(title= list(x = 0.05,
...
London_SF_prim_schools_agg_ind2009_borough_lines <- agg_input_london_SF_primary_schools_borough %>%
   group_by(la_name.x) %>% 
   mutate(indexed_headcount = (agg_full_time/first(agg_full_time))*100) %>% 
...
ggplotly(London_SF_prim_schools_agg_ind2009_borough_lines, tooltip = "text") %>%
  style(hoverlabel = list(bgcolor = "white")) %>%
  layout(title= list(x = 0.05,
...
regions_SF_primary_schools_lines <- region_SF_primary_schools %>%
  ...
  labs(title= "State-funded full-time primary school pupils, by region of England, 2009 - 2022", 
       caption = paste0("Source: ONS, Chart: GLA demography"))
regions_SF_primary_schools_lines

```



```{r fig_Rest_ofEng_Indep_Prim_schools_line_1, fig.height = 5.56, fig.width = 9,   echo=FALSE, warning=FALSE, message=FALSE}

my_pal = gla_pal(palette_type = "categorical",
        palette_name = "core", n = 9)

# Now plot independent schools data by English region

# Aggregate by region 

region_indep_primary_schools <- input_all_schools_data %>%
  filter(la_name.x != "City of London") %>%
  filter(age < 11) %>%
  filter(type == "Independent") %>%
  group_by(period_start, region_name) %>% 
...
regions_indep_primary_schools_lines <- region_indep_primary_schools %>%
   ggplot(aes(x = period_start, y = agg_full_time, group = region_name, color = region_name,
             text = paste("Year  :", period_start,
                            "<br>Zone : ", region_name,
                            "<br>Headcount : ", round(agg_full_time, digits = 0))
  ...
  geom_line(size = 1) + 
    scale_y_continuous(labels = label_number(suffix = "K", scale = 1e-3, big.mark = ","))+
  scale_color_manual(name='Zone:', values = my_pal) +
  labs(title= "Independent primary school pupils, by region of England, 2009 - 2022", 
       caption = paste0("Source: ONS, Chart: GLA demography"))
regions_indep_primary_schools_lines
...
```{r fig_Rest_ofEng_Indep_Prim_schools_line_2, fig.height = 5.56, fig.width = 9,   echo=FALSE, warning=FALSE, message=FALSE}

# Plot proportion of independent schools by region
...
schools_indep_region_wide <- input_all_schools_data %>%
  filter(la_name.x != "City of London") %>%
  filter(age < 11) %>%
  pivot_wider(
    names_from = type,
...
regions_indep_primary_schools_lines <- schools_indep_region_wide %>%
   ggplot(aes(x = period_start, y = indepProp, group = region_name, color = region_name,
             text = paste("Year  :", period_start,
                            "<br>Zone : ", region_name,
                            "<br>Headcount : ", round(indepProp, digits = 0))
  )) +
  ...
  geom_line(size = 1) + 
   # scale_y_continuous(labels = label_number(suffix = "K", scale = 1e-3, big.mark = ","))+
  scale_color_manual(name='Zone:', values = my_pal) +
  labs(title= "Proportion of Independent primary school pupils, by region of England, 2009 - 2022", 
       caption = paste0("Source: ONS, Chart: GLA demography"))
regions_indep_primary_schools_lines
...
# Index aggregated values for independent schools to 2009 value

region_ind2009_lines <- region_indep_primary_schools %>%
 ...
 # scale_y_continuous(breaks = seq(88, 102, 2)) + #, limits = c(88, 102)) +
 scale_color_manual(name='Zone:', values = my_pal) +
  labs(title= "Independent full-time primary school pupils, by regions of England, 2009 - 2022, indexed to 2009", 
       caption = paste0("Source: ONS, Chart: GLA demography"))
...
# Aggregate SF prim schools to two categories, London and the rest of England, and index to 2009 value
...
London_orNot_SF_2009_lines <- London_orNot_SF %>%
   group_by(london_factor) %>% 
    mutate(period_start_num = as.numeric(period_start)) %>%
   mutate(indexed_headcount = (agg_agg_full_time/first(agg_agg_full_time))*100) %>% 
   ggplot(aes(x = period_start_num, y = indexed_headcount, group = london_factor, color = london_factor
             , text = paste("Year  :", period_start,
 ...
 scale_colour_manual(values=c("#943fa6", "#5ea15d"), 
                       name="Zone") +
labs(title= "Change in independent full-time primary school pupils, 2009 - 2022",
            subtitle = "Indexed to 2009", 
       caption = paste0("Source: ONS, Chart: GLA demography"))
London_orNot_SF_2009_lines
...
# Add rest of England indexed plot to London zone indexed plot for SF schools

Inner_London_SF_schools_agg_ind2009 <- agg_input_london_SF_primary_schools %>%
...
Outer_London_SF_schools_agg_ind2009 <- agg_input_london_SF_primary_schools %>%
...
ggsave (filename = (paste0(chart_dir, "Schools_Prim_LondonZone_orNot_SF_2009_lines.png")),
...
```{r fig_Rest_ofEng_Prim_schools_line_4_B, fig.height = 5.56, fig.width = 9,   echo=FALSE, warning=FALSE, message=FALSE}


LondonZone_orNot_SF_2009_lines_summ <-
   ggplot() +
  theme_gla() +
  geom_line(data = Inner_London_SF_schools_agg_ind2009, aes(x = period_start_num, 
                                                         y = indexed_headcount, group = 1), 
...
ggsave (filename = "C:/Families/Charts_for_Daryl/Summ1_Schools_Prim_LondonZone_orNot_SF_2009_lines.svg",
         plot = LondonZone_orNot_SF_2009_lines_summ,
         device = "svg",
         dpi=600,
         width = 9,
         units = "in")

LondonZone_orNot_SF_2009_lines_rep <-
   ggplot() +
  theme_gla() +
  geom_line(data = Inner_London_SF_schools_agg_ind2009, aes(x = period_start_num, 
                                                         y = indexed_headcount, group = 1), 
...
ggsave (filename = "C:/Families/Charts_for_Daryl/12_A_Schools_Prim_LondonZone_orNot_SF_2009_lines.svg",
         plot = LondonZone_orNot_SF_2009_lines_rep,
         device = "svg",
         dpi=600,
         width = 9,
         units = "in")
